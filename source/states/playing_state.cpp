...
void PlayingState::update( Input &input, double delta_time ) {
	...
	// BLOCK BREAKING
	if(input.get_mouse_state().first && block_lock == 0.0) { // if the player left-click and is not locked
		// cast a ray
		...
		for(int i = 0; i < 25 && !done; ++i) {
			...
			// if we find a block
			if(m_world.set_block(ray_pos.x, ray_pos.y, ray_pos.z) != Blocks::AIR) {
				// destroy it, stop the ray cast, and reset "block_lock"
				m_world.set_block(ray_pos.x, ray_pos.y, ray_pos.z, Blocks::AIR);
				done = true;
				block_lock = 0.25;
			}
		}
	...
	if(input.get_mouse_state().second && block_lock == 0.0) { // if the player right-click and is not locked
		...
		for(int i = 0; i < 25 && !done; ++i) {
			...
			if(m_world.set_block(ray_pos.x, ray_pos.y, ray_pos.z) != Blocks::AIR) {
				...
				// place a brick block
				m_world.set_block(ray_pos.x, ray_pos.y, ray_pos.z, Blocks::BRICK);
				done = true;
				block_lock = 0.25;
			}
		}
